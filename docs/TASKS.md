...
- **T20. Инструмент должен останавливать вызов процедуры, меняющей состояние.** `дальше`.
  `safety.py` классифицирует VQL по первому ключевому слову, и разрушительными считаются
  ровно `DROP`, `ALTER`, `DELETE`, `TRUNCATE`. Поэтому `SELECT * FROM DROP_REMOTE_TABLE(…)`
  и `CALL CLEAN_CACHE_DATABASE(…)` возвращаются с `destructive: null` и на боевом профиле
  проходят **без** `--allow-destructive`, то есть без остановки на подтверждение человека.
  Из всех открытых вопросов этот единственный оплачивается чужими данными, а не временем:
  текст навыка агент может не прочитать, отказ инструмента обойти нельзя.

  **Список уже есть и вести руками его заново не надо** — T14 вынесла его в
  `skills/procedures/references/predefined.md`: `GENERATE_STATS`, `CREATE_REMOTE_TABLE`,
  ...
  `ROLLBACK_ICEBERG_VIEW_TO_SNAPSHOT`, `UNLOCK_LOCAL_REPOSITORY`. Работа: `classify_vql`
  достаёт имя процедуры из `SELECT … FROM <NAME>(` и `CALL <NAME>(` и сверяет с этим
  списком. Чёрный список, а не белый — иначе `GET_ELEMENTS()` и прочие чтения начнут
  ложно срабатывать.

  **Решить внутри задачи:** где живёт список, чтобы код и навык не разошлись — сейчас он
  написан прозой в справочнике, а нужен машиночитаемым. Вариант «источник в коде, навык
  ссылается» ломает правило «навык самодостаточен»; вариант «две копии» ломается молча.

  **Граница, которую записать сразу:** пользовательская VQL-процедура, делающая DDL через
  `EXECUTE` внутри себя, по имени не ловится. Задача закрывает предопределённые процедуры
  платформы, а не произвольный код, и в навыке это должно остаться сказанным.

  Проверка: юнит-тесты на классификацию обеих форм вызова, и на стенде — отказ на профиле
  с `production = true` без флага и проход с флагом.

---

## Открытые вопросы

- Вызов процедуры, меняющей состояние, инструмент не считает разрушительным.
  `safety.py` классифицирует VQL по первому ключевому слову, поэтому
  `SELECT * FROM DROP_REMOTE_TABLE(…)`, `CALL CLEAN_CACHE_DATABASE(…)` и `GENERATE_STATS`
  возвращаются с `destructive: null` и на боевом профиле проходят без
  `--allow-destructive` (проверено на коде и на стенде в T14). Ровно та же ловушка, что у
  `POST`-ов маркетплейса, которую раздел 6.3 спеки уже признаёт, — но в канале VQL. Навык
  `procedures` предупреждает об этом текстом, инструмент — нет. Починка требует списка
  процедур, меняющих состояние. Список нашёлся написанным — T14 оставила его в
  `references/predefined.md`, — поэтому вопрос **заведён задачей T20**.
...
- Объём `/denodo:query` в v1 остаётся минимальной дельтой (раздел 9 спеки). T1 его не
  ...
  **T13 показал, что замыкается:** все три приёмочных сценария прошли без навыка `query`
  ...
  фактов наберётся больше, им место в `query`. **T19 добавил к ней третью** — тот же рецепт
  над `decimal` сам портит деньги, — и заодно снял побочный симптом: три ссылки на
  несуществующий `/denodo:query` из текстов убраны, так что вопрос теперь ровно один —
  заводить навык или расселять дельту по месту, — и тупиков в текстах он больше не создаёт.
