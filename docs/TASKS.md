...
- **T19. Приёмка вехи B: от фразы до публикации в маркетплейсе.** `сейчас`. Два
  последовательных прогона субагентами, каждому — одна фраза и папка со всеми навыками
  (копии в scratchpad, метод T13). Прогон A: витрина из демо-файлов, которых нет ни в одном
  шаблоне, и её разметка в маркетплейсе тегом и категорией — меряется развилка «тег VDP
  против тега маркетплейса» и стык двух каналов в одной задаче. Прогон B: внешний элемент
  целиком, поверх витрины прогона A. Последовательно, потому что на маркетплейсе своей базы
  не существует. Закрытие задачи закрывает веху B.
...
- Вызов процедуры, меняющей состояние, инструмент не считает разрушительным.
  ...
  процедур, меняющих состояние, то есть данных, которые придётся вести руками. Задачи нет.
