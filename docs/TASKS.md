...
замыкается на пустом стенде, без внешней СУБД. `scripts/denodo verify` (T12) уже гоняет
ровно эту цепочку до производного представления — T13 может опереться на него как на
готовый прогон, а не собирать сценарий с нуля.
...
### T16. `serverId` маркетплейса не настраивается

**Статус:** дальше
**Веха:** B

`Profile.marketplace_server_id` нигде не читается: `serverId` в
`verification/chain.toml` (и в любом другом месте, где он нужен) — захардкоженное
значение demo-стенда (`306`). Форк, указывающий на другой маркетплейс, не имеет чистого
способа его переопределить, кроме правки манифеста руками. Найдено при закрытии T12.
...
- **T12. `scripts/denodo verify`.** Подкоманда гоняет план из `verification/chain.toml`
  ...
  T1 — база каталога, папки, DF/CSV-источник, производные представления, интерфейсное
  представление, ассоциация, теги, — оба канала, VQL и HTTP, с уборкой в `try/finally` и
  простановкой пометок по `--update-marks`. Marketplace-хвост (синхронизация каталога,
  тег, категория) — по флагу `--with-marketplace`, по умолчанию выключен. Флаги: `--env`,
  `--chain`, `--database`, `--with-marketplace`, `--keep`, `--update-marks`,
  `--allow-destructive`.
  ...
  **Стенд поправил раздел 11.1 спеки.** `GET_SERVER_INFO()` на 9.5.1 не существует
  ...
  259 пройдено, 12 пропущено без стенда; интеграционный
  ...
  стоят по разовым прогонам T8b, и внешний элемент маркетплейса — T18;
  `Profile.marketplace_server_id`, нигде не читаемый, из-за чего `serverId` в манифесте
  зашит — T16; создание вместо сверки в marketplace-хвосте, из-за чего второй прогон над
  оставленными `--keep` объектами падает на `409`, — T17, сознательно не исправлено в
  рамках T12. T13 (сквозной сценарий) может опереться на `verify` как на готовый прогон.
