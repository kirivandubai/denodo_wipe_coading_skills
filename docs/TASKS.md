...
Объём сократился до одной подкоманды: шаблоны всех четырёх доменных навыков проставлены
`verified:` по ходу T8a–T8d на demo-стенде 9.5.1, включая marketplace-часть. Остаётся
`scripts/denodo verify` как повторяемый прогон — и он единственный, кому нужен формат
шаблонов сразу двух каналов, VQL и HTTP.
