...
**Статус:** дальше (T8a–T8d закрыты, блокировка снята)
**Веха:** B
...
Сюда же — **контрольный прогон приёмки на установленном плагине**: T13 гонял навыки
копиями в scratchpad, то есть мерил исполнение, а не срабатывание по `description`.
`claude plugin eval` всё равно требует установленного плагина, так что одна фраза из
сценария A, доведённая до витрины в свежей сессии после установки, обходится здесь почти
бесплатно и закрывает разницу между «навык применён» и «навык выбран сам».
