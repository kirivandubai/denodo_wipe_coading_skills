...
/denodo:catalog       виртуальные базы, папки, теги
/denodo:datasources   источники, wrappers, base views
/denodo:views         derived views, interface views
...
репозиторий-каталог не нужен. Установка:
`/plugin marketplace add kirivandubai/denodo_wipe_coading_skills`, затем
`/plugin install denodo@<маркетплейс>`. Имя репозитория на GitHub отличается от имени
...
стандартами. Формат файла фиксируется в плане реализации.
...
Правила написания VQL, переживающего повторное применение. Конкретный набор
поддерживаемых Denodo 9.5 конструкций (`CREATE OR REPLACE`, `IF NOT EXISTS` и их
применимость к разным типам объектов) уточняется на стенде.
...
scripts/denodo verify    --env dev            прогон шаблонов, см. раздел 11.1
```
...
- **Профили сред лежат вне репозитория проекта** — по умолчанию `~/.denodo/profiles.toml`,
  с возможностью указать другой путь переменной окружения. Так креденшелы не попадают в
  git ни случайно, ни намеренно. Конкретный формат фиксируется в плане реализации.
...
- Ошибки Denodo — в `references/errors.md`: код → что означает на самом деле → что
  чинить. Читается в момент неудачной итерации и окупается быстрее прочих файлов.
...
### 11.1 Верификация VQL: работают ли шаблоны
...
обновляет пометки `verified:` в файлах.
...
**Входит:** ядро, слой исполнения, `catalog` (виртуальные базы, папки, теги и прочая
база, нужная почти в любом проекте), `datasources` (источники, wrappers, base views),
`views` (derived, interface), `query` в объёме минимальной дельты, верификация и
eval-сьют.

Точный перечень объектов внутри `catalog` определяется отдельной задачей до начала
реализации.
...
**Риск №1 — пропускная способность канала для DDL.** Официально `denodo-sqlalchemy`
предназначен для запросов. Позволяет ли соединение на порту 9996 выполнять полноценный
DDL (`CREATE DATASOURCE`, `CREATE WRAPPER`), или часть операций придётся уводить в
REST-транспорт, — неизвестно. Права пользователя на стенде тоже влияют на результат.

**Первый шаг реализации — спайк:** подключиться к живому стенду и выполнить по очереди
`SELECT 1`, `CREATE VIEW`, `CREATE DATASOURCE`. Ответ определяет, насколько нагружен
`api_rest` в v1.

**Открытый вопрос:** точный перечень объектов `catalog` для v1 (отдельная задача).

**Уточняется на стенде:** набор поддерживаемых Denodo 9.5 конструкций идемпотентности
по типам объектов.
