...
`-- verified: <версия> (стенд, <дата>)` только у успешных шагов вида `template` и только
внутри блока кода, из которого шаг читает тело; версия берётся из ответа сервера. Те же
факты, повторённые прозой рядом с таблицами (`*verified: 9.5.1 (стенд, …)*`), флаг не
