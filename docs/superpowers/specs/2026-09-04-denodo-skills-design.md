...
списком стирает остальные импортированные теги, `element-management/all/synchronize` с
`proceedWithConflicts:"SERVER"` затирает локальные правки, а `views/{id}/tags` заменяет
набор тегов представления, а не дополняет его. Перечень — раздел 6
[отчёта T11](2026-09-08-spike-t11-marketplace-api.md).
...
(спайк T11) — это пять HTTP-вызовов и VQL-фрагмент: тип элемента, тип провайдера, custom
external tool server, затем interface view в VDP по контракту, который выдаёт сам
маркетплейс, и синхронизация. Блок 2 такого навыка — вся цепочка в порядке выполнения,
пометка `verified:` относится к цепочке целиком, а не к каждому вызову по отдельности.
...
`serverId` необязателен при одном сервере, импортированные из VDP теги в маркетплейсе
