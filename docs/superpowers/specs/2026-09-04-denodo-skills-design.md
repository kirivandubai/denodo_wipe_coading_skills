...
Конструкция `IF EXISTS` встречена в документации только у `DROP TAG`; её применимость к
остальным типам, как и поведение `CREATE OR REPLACE` на живом сервере, проверяется на
стенде.

Для объектов маркетплейса идемпотентность обеспечивается не синтаксисом, а порядком
вызовов: перед созданием — поиск по имени.
...
**Риск №2 — недокументированный API маркетплейса.** Публичная документация 9.5 описывает
базовый URL, способы аутентификации и параметр `serverId`, но не пути и не тела запросов:
они доступны только через `swagger-ui` на самом сервере. Шаблоны для тегов, категорий и
external elements нельзя написать по документации — им нужен доступ к живому Data
Marketplace. Отсюда отдельный спайк и вынос marketplace-части за первую веху.

**Закрыто:** точный перечень объектов v1 — см. [объём v1](2026-09-04-denodo-v1-scope.md).

**Уточняется на стенде:** поведение `CREATE OR REPLACE` на живом сервере и применимость
`IF EXISTS` к типам объектов кроме тегов.
